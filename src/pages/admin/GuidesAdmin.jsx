...
export default function GuidesAdmin() {
  ...
  const handleUpload = async (file) => {
    ...
    if (file.type !== "application/pdf") {
      setMsg({ ok: false, text: "يُقبل ملف PDF فقط." });
      return;
    }
    setUploading(true);
    setMsg(null);
    try {
      ...
      if (!form.title.trim()) set("title", file.name.replace(/\.pdf$/i, ""));
    } catch (e) {
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm leading-relaxed text-muted">
          أدلة PDF تُعرض في صفحة الأدلة العامة، مصنّفة حسب الفئة المستهدفة.
        </p>
      </div>
      ...
      <section className="card space-y-4 p-4">
        ...
        <div>
          <label className="text-xs text-muted">ملف الدليل (PDF)</label>
          <input type="file" accept="application/pdf" className="mt-1 block w-full text-sm"
